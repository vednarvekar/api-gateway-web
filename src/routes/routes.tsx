...
export const Route = createFileRoute("/routes")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Manage dynamic proxy routes, auth, and roles." },
    ],
  }),
    ...
    <div className="px-6 py-6 flex flex-col gap-4">
      ...
      <div className="rounded-lg border bg-card overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {isLoading && (
              <tr><td colSpan={9} className="px-4 py-8 text-center text-muted-foreground text-xs">Loading routes…</td></tr>
            )}
            {data?.map((r) => (
              <tr key={r.id} className="border-b border-border/60 last:border-0 hover:bg-accent/40 transition-colors">
                <td className="px-4 py-3 font-mono-num text-foreground">{r.path}/*</td>
                <td className="px-4 py-3 font-mono-num text-muted-foreground text-xs">{r.upstream}</td>
                <td className="px-4 py-3">
                  <Badge variant="outline" className="font-mono-num text-[10px] uppercase tracking-wider">
                ...
                <td className="px-4 py-3">
                  <div className="flex flex-wrap gap-1">
                    ...
                    {r.roles.map((role) => (
                      <span key={role} className="font-mono-num text-[10px] px-1.5 py-0.5 rounded bg-muted text-muted-foreground border border-border">
                        {role}
                      </span>
                    ))}
                  </div>
                </td>
                <td className="px-4 py-3 text-right font-mono-num">{formatNumber(r.requests24h)}</td>
                <td className="px-4 py-3 text-right font-mono-num">
                  <span className={r.errorRate > 0.03 ? "text-destructive" : r.errorRate > 0.01 ? "text-warning" : "text-muted-foreground"}>
                    {(r.errorRate * 100).toFixed(2)}%
                  </span>
                </td>
                <td className="px-4 py-3 text-right font-mono-num text-muted-foreground">{r.p95}ms</td>
                <td className="px-4 py-3">
                  <StatusPill
